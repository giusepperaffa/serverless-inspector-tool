/**
* @name Identify multiple write-like s3 actions configuration
* @description This query identifies when multiple write-like s3 actions are specified
*              in the infrastructure code
* @kind problem
* @problem.severity warning
* @id python/iac-s3-multiple-write-like-action
* @tags security
*/
...
predicate check_action_key(Dict d) {
  ...
  and
  forall(string myElem | myElem = d.getAValue().(Dict).getAValue().(Dict).getAValue().(List).getAnElt().(StrConst).getText() | myElem.matches("s3:Put%"))
}
