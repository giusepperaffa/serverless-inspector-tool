/**
* @name Identify multiple action configuration with iam nested syntax
...
* @id python/iac-multiple-action-nested-syntax
* @tags security
*/
...
predicate check_action_key(Dict d) {
  ...
  and  
  count(string myString | myString = d.getAValue().(List).getAnElt().(Dict).getAValue().(List).getAnElt().(StrConst).getText() and (not myString.matches("arn:%")) | myString) > 1 
}
