/**
* @name Identify multiple delete-like dynamodb actions configuration with iam nested syntax
* @description This query identifies when multiple delete-like dynamodb actions are specified
*              in the infrastructure code (nested iam syntax used)
* @kind problem
* @problem.severity warning
* @id python/iac-dynamodb-multiple-delete-like-action-nested-syntax
* @tags security
*/
...
predicate check_action_key(Dict d) {
  ...
  and
  count(string myElem | myElem = d.getAValue().(List).getAnElt().(Dict).getAValue().(List).getAnElt().(StrConst).getText() and myElem.matches("dynamodb:Delete%") | myElem) > 1 
}
...
select d,  "Identified multiple delete-like dynamodb actions configuration" 
