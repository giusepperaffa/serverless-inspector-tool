/**
* @name Identify any dynamodb action configuration with iam nested syntax
...
* @id python/iac-dynamodb-any-action-nested-syntax
* @tags security
*/
...
predicate check_action_key(Dict d) {
  ...
  and
  d.getAValue().(List).getAnElt().(Dict).getAValue().(List).getAnElt().(StrConst).getText() = "dynamodb:*"
}
