/**
* @name Identify multiple write-like s3 actions configuration with iam nested syntax
* @description This query identifies when multiple write-like s3 actions are specified
*              in the infrastructure code (nested iam syntax used)
* @kind problem
* @problem.severity warning
* @id python/iac-s3-multiple-write-like-action-nested-syntax
* @tags security
*/
...
predicate check_action_key(Dict d) {
  ...
  and
  count(string myElem | myElem = d.getAValue().(List).getAnElt().(Dict).getAValue().(List).getAnElt().(StrConst).getText() and myElem.matches("s3:Put%") | myElem) > 1 
}
...
select d,  "Identified multiple write-like s3 actions configuration"
