/**
* @name Identify multiple read-like dynamodb actions configuration with iam nested syntax
...
* @id python/iac-dynamodb-multiple-read-like-action-nested-syntax
* @tags security
*/
...
predicate check_action_key(Dict d) {
  ...
  and
  count(string myElem | myElem = d.getAValue().(List).getAnElt().(Dict).getAValue().(List).getAnElt().(StrConst).getText() and myElem.matches("dynamodb:Get%") | myElem) > 1 
}
